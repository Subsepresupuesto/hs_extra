...
export default function LiberarClient() {
  ...
  return (
    <div className="space-y-6">
      ...
      <section className="bg-white border border-slate-200 rounded-xl p-6">
        <h2 className="text-base font-semibold text-slate-900 mb-4">Legajos liberados</h2>
        {cargando ? (
          <p className="text-sm text-slate-500">Cargando...</p>
        ) : liberados.length === 0 ? (
          <p className="text-sm text-slate-500">No hay legajos liberados.</p>
        ) : (
              ...
              <tbody>
                {liberados.map((l) => (
                  <tr key={l.id} className="border-b border-slate-100">
                    ...
                    <td className="py-2 pr-4">{l.periodo ?? "Sin vencimiento"}</td>
                    ...
                    <td className="py-2 pr-4 text-right">
                      <button
                        onClick={() => handleToggle(l.id, !l.activo)}
